...
7. Next candidates: a minimal map client (React Native or a throwaway web page) to click the slice; Claim + contest window (the first multiplayer rule); auth (the API currently trusts `factionId` in the request body).
