...
9. ~~Auth~~ — done: magic-link login + server-side bearer sessions (2026-07-03, [ADR-0004](./adr/0004-magic-link-auth-and-bearer-sessions.md)). `POST /auth/request-link` → single-use 15-min token → `POST /auth/verify` → 30-day session; the server derives the Faction from the session (`GET /me`), so `factionId` is gone from every request body and the action-spoofing hole is closed. Email delivery is a swappable port (dev logs the link). WebSocket stays an unauthenticated read-only firehose (documented follow-up).
10. Next candidates: Outpost founding at claimed Locations (control currently grants no yield without an Outpost); Raids or Sieges (the remaining conflict verbs); scoping/authenticating the WebSocket firehose.
